...
void insert(struct trienode *, char *);         //insert a string into a trie
void display(struct trienode*);                 //displays all the words
void display_length(struct trienode*, int);     //displays all the words of a particular length
void delete_trie(struct trienode*,char*);       //deletes a word from a trie
void display_prefix(struct trienode*,char*);    //print all	 prefixes of a word
int search(struct trienode *,char *);           //search for a word
...
struct trienode *getnode()      // returns an initilaised trie node
{
    int i;
...
void insert(struct trienode *root, char *key)   // inserts a word to the trie
{
    ...
    for(i=0;key[i]!='\0';i++)
    {
        index=key[i];           // get ASCII value of character 
        if(curr->child[index]==NULL)
        {
            curr->child[index]=getnode();   //a position points to a new trie node 
        }
...
void display(struct trienode *curr)
{
    ...
    for(i=0;i<256;i++)
    {
        ...
        {
            word[length++]=i;                   //word and length are global variables
            if(curr->child[i]->endofword==1)    //if end of word, print the word
            {
                printf("\n"); 
                for(j=0;j<length;j++)
                {
...
//displays words of particular length
