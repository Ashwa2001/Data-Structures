...
//recursive function to search for key in a BST	
...
int minimum(struct tnode *t)	//leftmost node is the minimum element
{			
	if(t == NULL)
		return -1;
	else
...
int maximum(struct tnode *t)//rightmost element is the maximum element
{
	if(t==NULL)
		return -1;
	else
...
int count(struct tnode *t)	//no. of nodes = no(LEFT) + no(RIGHT) + 1
{
	if(t==NULL)//empty tree
	{
		return 0;
...
int sum(struct tnode *t)	//no. of nodes = no(LEFT) + no(RIGHT) + 1
{
	if(t==NULL)//empty tree
	{
		return 0;
...
{		
	...
	else	// you are at the node to be deleted
	{
		...
		else
		{
			...
			t->data = p->data;
			t->right = tdelete(t->right,p->data);
		}
	}
}*/
...
struct tnode *rinsert(struct tnode *t, int x)		// whatever you return is stored in root
{
	struct tnode *new;		
	if(t==NULL)
	{
